# Nextuss — relay opcional de Telegram

Este worker es **100% opcional**. Nextuss funciona sin él: mientras tienes la app abierta en una
...
- `GET /<SECRET>/pending` — Nextuss llama aquí al abrirse: devuelve los mensajes pendientes.
- `POST /<SECRET>/clear` — Nextuss llama aquí tras procesarlos, para vaciarlos.
...
2. Crea el namespace de KV: `npx wrangler kv namespace create NEXTUSS_KV` — copia el `id` que te
...
4. `npm run deploy`. Apunta la URL que te da Cloudflare, algo como
   `https://nextuss-telegram-relay.tu-cuenta.workers.dev`.
5. Registra el webhook en Telegram (sustituye `<TOKEN>`, `<WORKER_URL>` y `<SECRET>`):
   ...
   ```
6. En Nextuss, Ajustes → Telegram → "URL del worker (opcional)", pega
